...
set ISOFile=..\input\Metal Gear Solid Special Disc [J].iso
set NESFile=..\input\Metal Gear [U].nes
...
if not exist "%NESFile%" (
	echo [INFO] "%NESFile%" was not found
	echo Did you name the ROM correctly and place it in the correct folder?
	echo Exiting in 10 seconds..
	C:\Windows\System32\timeout.exe /t 10 /nobreak >nul
	exit /b 0
...
bass\\win\\bass.exe ..\\src\\Main_tgc.asm
